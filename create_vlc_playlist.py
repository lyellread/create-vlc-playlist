...
# Set false to get files only from cwd.
...
class Videos:
    ...
    def remove_nonvideo_files(self, file_list):
        ...
        for index, file_name in enumerate(file_list[:]):
            ...
            if file_name.endswith(tuple(ext_list)) or file_name.endswith(
                tuple(ext.upper() for ext in ext_list)
            ):
                pass
            else:
    ...
    def edit_paths(self, video_files):
        ...
        for index in range(len(video_files)):
            video_files[index] = (
                "file:///" + os.path.join(video_files[index])
            ).replace("\\", "/")
        return video_files
